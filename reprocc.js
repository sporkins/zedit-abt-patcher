...
function () {
  function AlchemyPatcher(helpers, locals, patch, settings) {
    ...
    _defineProperty(this, "updateEffect", function (effectsHandle) {
      var mgef = xelib.GetLinksTo(effectsHandle, 'EFID');
      if (!mgef) return;
      var mgefHandle = xelib.GetWinningOverride(mgef);
...
function () {
  ...
  _createClass(WeaponPatcher, [{
    ...
    value: function addMeltdownRecipe(weapon) {
      ...
      if (xelib.HasArrayItem(weapon, 'KWDA', '', s.excludeFromMeltdownRecipes)) {
        return;
      }
      ...
      if (!input) {
        var ignoreLog = ["Nightingale Shortbow", "Unarmed", "Sorcerer's Fists"];

        if (!ignoreLog.includes(this.names[weapon])) {
      ...
      var newRecipe = xelib.AddElement(this.patchFile, 'Constructible Object\\COBJ');
      xelib.AddElementValue(newRecipe, 'EDID', "REP_MELTDOWN_".concat(this.names[weapon]));
      xelib.AddElement(newRecipe, 'Items');
      var ingredient = xelib.GetElement(newRecipe, 'Items\\[0]');
